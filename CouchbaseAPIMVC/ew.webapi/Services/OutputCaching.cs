...
namespace ew.webapi
{
    public class WebApiOutputCacheAttribute : ActionFilterAttribute
    {
        ...
        /// Constructor
        /// </summary>
        public WebApiOutputCacheAttribute(int Duration = 180)
            : this(true)
        {
            _timespan = Duration;
        }
                    ...
                    if (IsCacheable(filterContext))
                    {
                        ...
                        if (WebApiCache.Contains(_cachekey))
                        {
                            ...
                            if (val != null)
                            {
                                ...
                                var contenttype = (MediaTypeHeaderValue)WebApiCache.Get(_cachekey +
                            ":response-ct");
                                if (contenttype == null)
                                    contenttype = new MediaTypeHeaderValue(_cachekey.Split(':')[1]);
                                filterContext.Response.Content.Headers.ContentType = contenttype;

                                var header = (HttpResponseHeaders)WebApiCache.Get(_cachekey +
                            ":headers");
                                if (header != null)
                                {
                                    foreach (var item in header)
                                    {
                                        if (!filterContext.Response.Headers.Contains(item.Key))
                                        {
                                            filterContext.Response.Headers.Add(item.Key, item.Value);
                                        }
                                    }
                                }
                                return;
        ...
        public override void OnActionExecuted(HttpActionExecutedContext filterContext)
        {
            try
            {
                ...
                DateTime.Now.AddSeconds(_timespan));
                                WebApiCache.Set(_cachekey + ":headers", filterContext.Response.Headers, DateTime.Now.AddSeconds(_timespan));
                            }
                            else
                            {
                                ...
                                filterContext.Response.Content.Headers.ContentType,
                DateTime.Now.AddSeconds(_timespan));
                                WebApiCache.Add(_cachekey + ":headers", filterContext.Response.Headers, DateTime.Now.AddSeconds(_timespan));
                            }
                        }
                    }
                }
